  ...
  return (
    <main className="min-h-screen bg-[#FBF9F5] text-[#2A211D]">
      ...
      <section className="wrap grid grid-cols-1 gap-4 py-12 md:grid-cols-2 lg:grid-cols-3 lg:py-16">
        {posts.map((p, i) => (
          <Reveal key={p.slug}>
            <a href={`/blog/${p.slug}`} className="card group flex h-full min-h-[280px] flex-col justify-between rounded-2xl border border-[#2A211D]/10 bg-white p-6 lg:p-8">
              <div>
                <div className="flex items-center justify-between font-mono text-[11px] uppercase tracking-[1px] text-[#2A211D]/45">
                  <span>{p.date}</span>
                  <span>{p.read}</span>
                </div>
                ...
                <p className="mt-2 text-sm leading-relaxed text-[#2A211D]/60">{p.excerpt}</p>
              </div>
