...
export default function CtaBand({ img, title, sub }: { img: string; title: string; sub: string }) {
  return (
    <section className="py-[60px] lg:py-[80px]">
      <Reveal className={`${wrap} relative h-[600px] overflow-hidden rounded-2xl bg-[#101418] lg:h-[640px] lg:rounded-3xl`}>
        ...
        <div className="absolute bottom-4 left-4 right-4 lg:bottom-6 lg:left-6 lg:right-6">
          <div className="flex flex-col gap-3 rounded-xl bg-[var(--brand)] px-5 py-3 lg:flex-row lg:items-center lg:justify-between lg:gap-4 lg:rounded-2xl lg:px-8 lg:py-3.5">
            <p className="text-sm font-normal leading-[1.4] text-white/85 lg:whitespace-nowrap lg:text-base">{sub}</p>
