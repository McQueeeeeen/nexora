  ...
  return (
    <section id="reviews" className="relative flex min-h-[80vh] items-center overflow-hidden py-24 lg:min-h-[100vh] lg:py-40">
      <div className="relative mx-auto w-full max-w-[1100px] px-6 text-center lg:px-12">
        ...
        <blockquote key={i} className="tv5-fade mx-auto flex min-h-[200px] items-center justify-center text-balance text-[24px] leading-snug text-[#2A211D] sm:text-[36px] lg:min-h-[300px] lg:text-[56px]">
          {q.text}
        </blockquote>
        <div className="mt-8 flex flex-col items-center gap-2 lg:mt-12">
          {detailed && (
            <span className="flex h-14 w-14 items-center justify-center rounded-full border border-[#2A211D]/10 bg-[#2A211D]/5 font-mono text-lg font-bold text-[#2A211D]" aria-hidden>
              {initials}
            </span>
          )}
          <span className="text-[22px] font-medium text-[#2A211D] sm:text-[26px] lg:text-[32px]">{q.author}</span>
          <span className="font-mono text-[12px] uppercase tracking-[1px] text-[#2A211D]/60 lg:text-[13px]">{q.role}</span>
        </div>
