...
import { wrap, Tag, Reveal, Btn, Y } from "./ui";

// Базовая инфа об агентстве на главной — кто мы и почему нам доверяют.
export default function About() {
  return (
    <section id="about" className="border-b border-white/10 py-24 lg:py-32">
      <div className={`${wrap} grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-16`}>
        ...
        <div className="flex flex-col gap-4 lg:col-span-5">
          {agency.points.map((pt, i) => (
            <Reveal key={pt}>
              <div className="card flex items-center gap-4 rounded-2xl border border-white/10 bg-[#181818] p-6">
                <span className="font-mono text-2xl font-bold" style={{ color: Y }}>0{i + 1}</span>
